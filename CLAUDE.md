...
**Repo & hosting (2026-09-01):** GitHub repo is [`mfloresrdev/ai-proservices-corpwebsite`](https://github.com/mfloresrdev/ai-proservices-corpwebsite), deployed via the Vercel project `ai-proservices-corpwebsite` (Root Directory: `site`) with auto-deploy on push to `main`. Primary domain is `aiproservicescorp.com` (Vercel Domains). The former domain `boriconsulting.com` (also on Vercel Domains, from before the rebrand) is still assigned to the project but its long-term fate — kept as a redirect, or fully detached — hasn't been decided; check before assuming either. Security headers (CSP, HSTS, X-Frame-Options, etc.) live in `site/vercel.json` — it must stay inside `site/`, not the repo root, or Vercel's Root Directory setting will silently ignore it. If the repo or Vercel project is ever renamed again, update both this file and `site/README.md`'s Deploying section in the same change.
